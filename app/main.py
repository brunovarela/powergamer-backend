...
def init_database():
    ...
    with sqlite3.connect(DATABASE_URL) as conn:
        ...
        cursor.execute("""
            CREATE INDEX IF NOT EXISTS idx_player_name ON player_snapshots(name)
        """)
        cursor.execute("""
            CREATE INDEX IF NOT EXISTS idx_player_date ON player_snapshots(date)
        """)
        cursor.execute("""
            CREATE INDEX IF NOT EXISTS idx_player_rank ON player_snapshots(rank_position)
        """)
...
class TibiaTracker:
    ...
    def scrape_page(self, page_num: int) -> List[Player]:
        ...
        try:
            ...
            if not table:
                print(f"Tabela de ranking não encontrada na página {page_num}!")
                return []
            ...
            for row in rows:
                try:
                    ...
                    print(f"Erro ao processar linha do ranking página {page_num}: {e}")
                    continue
            
            print(f"Página {page_num} processada: {len(players)} players encontrados")
            return players
            ...
            print(f"Erro no scraping da página {page_num}: {e}")
            return []
    
    def scrape_all_pages(self, total_pages: int = 5) -> List[Player]:
        """Scraping de todas as páginas do ranking (Top 100)"""
        all_players = []
        
        for page in range(1, total_pages + 1):
            print(f"Fazendo scraping da página {page}/{total_pages}...")
        ...
        print(f"Scraping completo: {len(all_players)} players encontrados no total")
        return all_players
...
def daily_scraping_job():
    ...
    players = tracker.scrape_all_pages(5)  # Top 100 players (5 páginas)
...
async def get_server_stats():
    ...
    with get_db_connection() as conn:
        ...
        cursor.execute("""
            SELECT MAX(exp_gained) as highest_exp_today
            FROM daily_gains
            WHERE date = ?
        """, (today,))
